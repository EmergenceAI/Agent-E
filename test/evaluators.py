...
class HTMLContentEvaluator(Evaluator):
    ...
    async def __call__(
        ...
        Parameters:
            ...
            page (Page): The Playwright page object for the current webpage.
            client (CDPSession | None, optional): The Chrome DevTools Protocol session object. Not used in this evaluator.
            agent_answer (str | None, optional): Answer generated by the agent.
        ...
        dynamic_comparison = task_config["eval"].get("dynamic_comparison", False)  # Add flag for dynamic comparison
        comparison_strategy = task_config["eval"].get("comparison_strategy", "exact_match")  # Add field for comparison strategy
        min_required_matches = task_config["eval"].get("min_required_matches", 1)  # Add field for minimum required matches
        ...
        for target in targets:
            ...
            # use JS to select the element
                ...
                try:
                    ...
                    expected_answer = await page.evaluate(f"() => {locator}")
            ...
            # run program to call API
                ...
                expected_answer = eval(func)
            else:
                raise ValueError(f"Unknown locator: {locator}")

            expected_answer = html.unescape(expected_answer)

            if dynamic_comparison:  # Perform dynamic comparison
                if agent_answer is not None:
                    return self.evaluate_dynamic(agent_answer, expected_answer, comparison_strategy, min_required_matches)

            if "exact_match" in target["required_contents"]:
                required_contents = target["required_contents"]["exact_match"]
                cur_score = evaluate_exact_match(
                    ref=required_contents, pred=expected_answer
                )
    ...
    def evaluate_dynamic(self, agent_answer: str, expected_answer: Any, strategy: str, min_required_matches: int) -> float:
        """Evaluates the selected element with the answer based on the specified strategy.

        Parameters:
            agent_answer (str): The answer generated by the agent (test result).
            expected_answer (Any): The dynamic content to compare against.
            strategy (str): The evaluation strategy to use (exact_match, must_include, etc.).
            min_required_matches (int): The minimum number of required matches for the some_matches strategy.

        Returns:
            float: The evaluation score.
        """
        if strategy == "exact_match":
            return evaluate_exact_match(ref=expected_answer, pred=agent_answer)
        elif strategy == "must_include":
            if isinstance(expected_answer, list):
                score = 1.0
                for must_value in expected_answer: # type: ignore
                    score *= evaluate_must_include(ref=must_value, pred=agent_answer, tokenize=(len(expected_answer) == 1)) # type: ignore
                    print(">>> checking must include: ", must_value, "against: ", agent_answer, "Score: ", score) # type: ignore
                return score
            else:
                return evaluate_must_include(ref=expected_answer, pred=agent_answer)
        elif strategy == "some_matches":
            if isinstance(expected_answer, list):
                matches = sum(evaluate_must_include(ref=phrase, pred=agent_answer, tokenize=False) for phrase in expected_answer) # type: ignore
                return float(matches >= min_required_matches)
            else:
                return 0.0
        elif strategy == "fuzzy_match":
            return evaluate_fuzzy_match(ref=expected_answer, pred=agent_answer, intent="")
        else:
